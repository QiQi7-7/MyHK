...
- **受击优化**  
  - 功能描述：修复因帧率导致的丢判，现在敌人受击的间隔总为0.2s，不受帧率影响。
...
- **白吼优化**  
  - 功能描述：优化白吼的击退机制，使之同黑吼（左吼排斥右吼吸引）或者永远击退。
...
- **两击必杀**  
  - 功能描述：若一次攻击无法击杀敌人则将血量降为1，可以用于测试，在适当的时机击杀BOSS，避免使用debug频繁调整伤害以及使用kill all可能导致的问题。
...
- **硬直修复**  
  - 功能描述：修复部分BOSS的硬直bug，包括硬直滑行（表哥、席奥、梦表），硬直立刻起身（席奥、斯莱），芬达哥无法正常打出连击硬直，前辈飞天大草和硬直星爆。
...
- **物品回收修复**  
  - 功能描述：修复切换场景时召唤的物体不能正确回收的bug，如辐光光剑残留。
  - 补充说明：属于严重的设计失误，但是对日常影响较小，推荐开启。
...
- **芬达哥修复**  
  - 功能描述：修复芬达哥的已知bug，包括落地后水平位移、p2特殊钻地保留。
